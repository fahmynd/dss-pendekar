  ...
  const APBD = useMemo(() => {
    ...
    if (selectedTahun) {
      if (selectedKec) {
        ...
        if (kec) {
          if (!kec[`anggaran_${selectedTahun}`])
            kec[`anggaran_${selectedTahun}`] = 0;
          if (!kec[`realisasi_${selectedTahun}`])
            kec[`realisasi_${selectedTahun}`] = 0;
          if (!kec[`penyerapan_${selectedTahun}`])
            kec[`penyerapan_${selectedTahun}`] = 0;
          desa = parseInt(kec.online);
          anggaran = parseInt(kec[`anggaran_${selectedTahun}`]);
          realisasi = parseInt(kec[`realisasi_${selectedTahun}`]);
          penyerapan = parseFloat(kec[`penyerapan_${selectedTahun}`]).toFixed(
            2
          );
        }
      } else {
        list_kecamatan.forEach((d) => {
          if (!d[`anggaran_${selectedTahun}`])
            d[`anggaran_${selectedTahun}`] = 0;
          if (!d[`realisasi_${selectedTahun}`])
            d[`realisasi_${selectedTahun}`] = 0;
